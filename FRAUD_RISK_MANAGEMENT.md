# GlidePay — Fraud Risk Management Statement
...
GlidePay is a B2B payroll SaaS platform serving US-based businesses. We process ACH payroll disbursements on behalf of employer connected accounts to their employees' bank accounts. This document describes the fraud controls we have in place for each risk category Stripe evaluates during Treasury for platforms approval.

---

## 1. Identity Fraud — Fake Companies

**Risk:** A bad actor creates a fraudulent company account on GlidePay to gain access to a Treasury Financial Account and initiate outbound transfers to accounts they control.

**Controls:**

- **Stripe KYB onboarding (primary):** Every company that wants ACH disbursement capabilities must complete Stripe's hosted account onboarding (`type: "custom"`), which collects business name, EIN, address, beneficial owner identity (SSN/DOB), and supporting documents. Stripe is the verifying party — GlidePay does not grant Treasury access until `stripe_account_status = "active"`, which only happens after Stripe's `account.updated` webhook confirms all capabilities are `active` and `requirements.currently_due` is empty.

- **Subscription gate:** GlidePay requires an active paid subscription (Stripe Checkout, verified credit card) before a company can process payroll. This adds a real financial cost to account creation and filters out fully anonymous bad actors.
...
- **Dual alert emails on bank account change:** The moment a bank account is confirmed (`confirm_setup` action), GlidePay sends:
  ...
  2. An alert to the **company admin email** — "[GlidePay] Bank account changed — Employee Name" with the old and new last-4 digits.
  
  This gives a 3-day window to catch and reverse unauthorized changes before any funds move.

- **Stripe Financial Connections (no raw account numbers):** GlidePay never collects or stores routing and account numbers directly. Bank accounts are linked via Stripe.js `collectBankAccountForSetup`, which uses Stripe Financial Connections. Only the payment method ID and last-4 are stored. This means a database breach does not expose usable account credentials.
...
- **OutboundTransfer description:** Every Stripe Treasury OutboundTransfer includes a `description` of `"GlidePay payroll — run {payrollRunId}"` and `metadata` containing `company_id`, `employee_id`, and `payroll_run_id`, making disputes traceable to the specific payroll run.

- **Webhook-driven status tracking:** `treasury.outbound_transfer.posted`, `failed`, and `returned` events update the `ach_transfers` table in real time and write audit log entries. Returns are immediately visible to admins in the GlidePay interface.
...
- **Subscription seat-count billing:** GlidePay's billing model charges per employee seat. Adding fictitious employees to inflate a payroll run also increases the monthly invoice, creating a financial disincentive and an audit signal.
...
- **Connected account isolation:** Each company's Treasury Financial Account belongs to their own Stripe connected account. GlidePay (the platform) cannot initiate OutboundTransfers from a connected account's Financial Account except via API calls made in that account's context — there is no pooled float held by GlidePay.
...
*This document reflects controls implemented in the GlidePay codebase as of June 2026. Controls are enforced in application code and Supabase database policy — not solely reliant on process.*
